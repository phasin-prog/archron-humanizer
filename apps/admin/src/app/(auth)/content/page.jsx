...
const HEADERS = ["Title", "Type", "Author", "Date", "Status", "Actions"];
...
function ContentTable({ items, onArchive, onDelete }) {
    if (items.length === 0) {
      ...
      <table className="w-full text-body">
        <thead>
          <tr className="border-b bg-muted/20">
            {HEADERS.map((h) => (<th key={h} className="px-4 py-3 text-left text-caption font-medium text-muted-foreground">
                {h}
              </th>))}
          </tr>
        </thead>
        <tbody>
          {items.map((item) => (<tr key={item.id} className="border-b transition-colors hover:bg-muted/20">
              ...
              <td className="px-4 py-3">
                <div className="flex items-center gap-1">
                  ...
                  {item.status === "published" && (<Button variant="ghost" size="sm" onClick={() => onArchive(item.id)}>Archive</Button>)}
...
export default function ContentPage() {
    ...
    const filterBySearch = (list) => search ? list.filter((i) => i.title.toLowerCase().includes(search.toLowerCase()) || i.slug.toLowerCase().includes(search.toLowerCase())) : list;
    const toggleAll = useCallback((list) => {
        setSelectedIds((prev) => {
            ...
            const next = new Set(prev);
            list.forEach((i) => { if (!next.has(i.id))
                next.add(i.id); });
            return next;
        });
    }, []);
    const archiveSelected = useCallback(() => {
        setItems((prev) => prev.map((i) => (selectedIds.has(i.id) && i.status === "published" ? { ...i, status: "archived" } : i)));
    ...
    const handleArchive = useCallback((id) => {
        setItems((prev) => prev.map((i) => (i.id === id && i.status === "published" ? { ...i, status: "archived" } : i)));
    }, []);
    const handleDelete = useCallback((id) => {
        setItems((prev) => prev.filter((i) => i.id !== id));
        setSelectedIds((prev) => { const next = new Set(prev); next.delete(id); return next; });
    }, []);
    const renderTab = (list, tabValue) => {
        const filtered = filterBySearch(list);
        const allSelected = filtered.length > 0 && filtered.every((i) => selectedIds.has(i.id));
        return (<TabsContent value={tabValue}>
        <div className="space-y-3">
          <div className="flex items-center gap-4">
            <Input placeholder="Search by title or slug..." value={search} onChange={(e) => setSearch(e.target.value)} className="max-w-xs"/>
            <div className="flex-1"/>
            {selectedIds.size > 0 && (<div className="flex items-center gap-2 rounded-md border px-3 py-1.5 text-caption">
          ...
          <Card>
            <CardContent className="p-0">
              <div className="flex items-center gap-3 border-b px-4 py-2">
                <input type="checkbox" checked={allSelected} onChange={() => toggleAll(filtered)} className="h-4 w-4"/>
    ...
    return (<div className="mx-auto max-w-6xl space-y-6 p-6">
      ...
      <Tabs defaultValue="published" onValueChange={useCallback((v) => setTab(v), [])}>
        <TabsList>
