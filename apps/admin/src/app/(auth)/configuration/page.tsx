...
interface ConfigForm {
  ...
  minWriterReputation: string
  minReviewerReputation: string
  minEditorReputation: string
...
export default function ConfigurationPage() {
  ...
  const update = (field: keyof ConfigForm, value: string | boolean | number) => {
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <Input
              ...
              placeholder="A human knowledge platform"
            />
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            {savedMessages.general && (
              <span className="text-sm text-green-600">{savedMessages.general}</span>
            )}
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Input
              ...
              min={1}
              max={100}
              value={String(form.maxDrafts)}
              onChange={(e) => update("maxDrafts", Number(e.target.value))}
            />
          </div>
          <div>
          ...
          <div>
            ...
            <Input
              ...
              min={1}
              max={60}
              value={String(form.reviewDeadline)}
              onChange={(e) => update("reviewDeadline", Number(e.target.value))}
            />
          </div>
          <div className="flex items-center gap-3">
            <Button onClick={() => handleSave("content")}>Save</Button>
            {savedMessages.content && (
              <span className="text-sm text-green-600">{savedMessages.content}</span>
            )}
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div>
            ...
            <Select
              options={roleOptions}
              value={form.minWriterReputation}
              onChange={(e) => update("minWriterReputation", e.target.value)}
            />
          </div>
          <div>
            ...
            <Select
              options={roleOptions}
              value={form.minReviewerReputation}
              onChange={(e) => update("minReviewerReputation", e.target.value)}
            />
          </div>
          <div>
            ...
            <Select
              options={roleOptions}
              value={form.minEditorReputation}
              onChange={(e) => update("minEditorReputation", e.target.value)}
            />
          </div>
          <div className="flex items-center gap-3">
            <Button onClick={() => handleSave("roles")}>Save</Button>
            {savedMessages.roles && (
              <span className="text-sm text-green-600">{savedMessages.roles}</span>
            )}
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div>
            ...
            <Input
              ...
              placeholder="https://hooks.example.com/archron"
            />
          </div>
          <div className="flex items-center gap-3">
            <Button onClick={() => handleSave("notifications")}>Save</Button>
            {savedMessages.notifications && (
              <span className="text-sm text-green-600">{savedMessages.notifications}</span>
            )}
          </div>
        </CardContent>
      </Card>
