    ...
    const filtered = useMemo(() => {
        return PLACEHOLDER_ENTRIES.filter((entry) => {
            if (startDate && entry.timestamp < startDate)
                return false;
            if (endDate && entry.timestamp > endDate + "T23:59:59Z")
                return false;
        ...
        <CardContent>
          <div className="flex flex-wrap gap-4">
            ...
            <div className="w-48">
              ...
              <Select options={actionOptions} value={actionFilter} onChange={(e) => { setActionFilter(e.target.value); handleFilterChange(); }}/>
            </div>
            <div className="w-56">
              <label className="mb-1 block text-xs text-muted-foreground">Actor</label>
              <Input placeholder="Search by email..." value={actorFilter} onChange={(e) => { setActorFilter(e.target.value); handleFilterChange(); }}/>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {paginated.map((entry) => (<tr key={entry.id} className="border-b last:border-b-0 hover:bg-muted/30 transition-colors">
