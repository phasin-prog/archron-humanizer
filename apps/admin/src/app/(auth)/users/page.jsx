...
export default function AdminUsersPage() {
    return (<div className="mx-auto max-w-6xl space-y-6 p-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <Select options={ROLE_OPTIONS} placeholder="All Roles" className="w-40"/>
      </div>
        ...
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {USERS.map((user) => (<tr key={user.id} className="border-b last:border-0 hover:bg-muted/30">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2">
                        <Link href={`/admin/users/${user.id}`} className="text-caption text-primary hover:underline">
                          View
                        </Link>
                        <Link href={`/admin/users/${user.id}`} className="text-caption text-primary hover:underline">
                          Edit
                        </Link>
                        {user.status === "active" ? (<button className="text-caption text-destructive hover:underline">
                            Suspend
                          </button>) : (<button className="text-caption text-primary hover:underline">
                            Reactivate
                          </button>)}
                      </div>
                    </td>
