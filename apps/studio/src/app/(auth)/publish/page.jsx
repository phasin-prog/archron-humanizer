...
export default function PublishPage() {
    return (<div className="mx-auto max-w-6xl space-y-6 p-6">
      ...
      <div className="grid grid-cols-3 gap-6">
        {COLUMNS.map((column) => (<div key={column.status} className="space-y-3">
            ...
            {column.items.map((item) => (<Card key={item.id}>
                <CardHeader>
                ...
                <CardContent>
                  ...
                  <div className="mt-3 flex gap-2">
                    ...
                    {column.status === "published" && (<Button size="sm" className="flex-1">Publish</Button>)}
                  </div>
                </CardContent>
              </Card>))}
          </div>))}
