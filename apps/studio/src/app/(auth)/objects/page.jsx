...
export default function ObjectsPage() {
    ...
    const filtered = placeholderObjects.filter((obj) => {
        if (search && !obj.title.toLowerCase().includes(search.toLowerCase()))
            return false;
        if (typeFilter !== "all" && obj.objectType !== typeFilter)
            return false;
        return true;
    });
    return (<div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="flex gap-4 mb-6">
        ...
        <Select options={typeOptions} value={typeFilter} onChange={(e) => setTypeFilter(e.target.value)} className="max-w-[180px]"/>
        <Select options={statusOptions} value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="max-w-[180px]"/>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {filtered.map((obj) => (<KnowledgeCard key={obj.slug} {...obj}/>))}
      </div>
    </div>);
}
