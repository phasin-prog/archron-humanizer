...
export function ConstellationScene({
  ...
  const draw = useCallback(() => {
    ...
    for (const node of currentNodes) {
      ...
      const isDimmed =
        ...
        !currentEdges.some(
          (e) =>
            (e.source === currentHovered && e.target === node.id) ||
  ...
  return (
    <div
      ref={containerRef}
      className={cn("relative w-full h-[600px] overflow-hidden rounded-xl border border-border bg-background", className)}
    >
      <canvas
        ref={canvasRef}
        ...
        onClick={handleClick}
        style={{ cursor: isPanning ? "grabbing" : draggingId ? "grabbing" : hoveredId ? "pointer" : "grab" }}
      />
    </div>
  )
}
