...
import { PsychologyGlyph, PhilosophyGlyph, SchoolCard, ThinkerCard, ArticleCard, Breadcrumbs, BreadcrumbItem, BreadcrumbSeparator, } from "@archron/ui";
const disciplines = {
    psychology: {
        name: "Psychology",
        slug: "psychology",
      ...
      <div className="mx-auto max-w-container-page px-6 pb-24">
        ...
        <section className="mt-8">
          <div className="flex items-center gap-4">
            <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-xl" style={{ backgroundColor: `${data.color}15`, color: data.color }}>
              <Glyph size="lg"/>
            </div>
            <div>
        ...
        {data.schools.length > 0 && (<section className="mt-16">
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {data.schools.map((school) => (<SchoolCard key={school.slug} {...school}/>))}
            </div>
          </section>)}

        {data.thinkers.length > 0 && (<section className="mt-20">
            <h2 className="mb-6 font-serif text-section font-semibold text-text">
              Featured Thinkers
            </h2>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {data.thinkers.map((thinker) => (<ThinkerCard key={thinker.slug} {...thinker}/>))}
            </div>
          </section>)}

        {data.timeline.length > 0 && (<section className="mt-20">
            <h2 className="mb-6 font-serif text-section font-semibold text-text">
              Timeline
            </h2>
            <div className="overflow-x-auto rounded-xl border border-border bg-card p-6">
              <div className="flex min-w-max gap-0">
                {data.timeline.map((event, i) => (<div key={i} className="flex items-center">
                    <div className="flex flex-col items-center">
                      <div className="h-3 w-3 rounded-full" style={{ backgroundColor: data.color }}/>
                      <span className="mt-1 font-mono text-meta font-medium" style={{ color: data.color }}>
                        {event.year}
                      </span>
                    ...
                    {i < data.timeline.length - 1 && (<div className="mx-6 mt-2 h-px w-16 bg-border"/>)}
                  </div>))}
              </div>
            </div>
          </section>)}

        {data.articles.length > 0 && (<section className="mt-20">
            <h2 className="mb-6 font-serif text-section font-semibold text-text">
              Latest Articles
            </h2>
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {data.articles.map((article) => (<ArticleCard key={article.slug} {...article}/>))}
            </div>
          </section>)}
      </div>
    </div>);
