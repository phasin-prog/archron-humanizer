...
import { KnowledgeConceptIcon, KnowledgeThinkerIcon, KnowledgeBookIcon, KnowledgeArticleIcon, } from "@archron/ui";
...
const TYPE_ORDER = ["concept", "thinker", "book", "article"];
const TYPE_ICONS = {
...
function filterResults(results, query, type) {
...
export default function SearchPage() {
    ...
    const filtered = useMemo(() => filterResults(PLACEHOLDER_RESULTS, query, activeType), [query, activeType]);
    ...
    return (<main className="mx-auto w-full max-w-container-page px-6 py-8">
      ...
      {noResults ? (<div className="flex flex-col items-center gap-4 py-16 text-center">
          ...
          <div className="mt-4 flex flex-wrap justify-center gap-2">
            {["Analytical Psychology", "Philosophy", "Psychoanalysis"].map((domain) => (<Link key={domain} href={`/explore/${domain.toLowerCase().replace(/\s+/g, "-")}`} className="rounded-lg border border-border bg-card px-4 py-2 text-caption text-text-muted transition-colors hover:border-primary/30 hover:text-text">
                {domain}
              </Link>))}
          </div>
          ...
          {grouped.map((group) => (<section key={group.type}>
              ...
              <div className="space-y-3">
                {group.items.map((item) => {
                    ...
                    return (<Link key={item.id} href={`/${item.slug}`}>
