...
import {
  ...
  type SceneNode,
  type SceneEdge,
} from "@/components/constellation/constellation-scene"
...
const PLACEHOLDER_NODES: SceneNode[] = [
...
const PLACEHOLDER_EDGES: SceneEdge[] = [
...
export default function ConstellationPage() {
  ...
  const filteredNodes = useMemo(
    () =>
      activeFilter === "all"
        ? PLACEHOLDER_NODES
        : PLACEHOLDER_NODES.filter((n) => n.type === activeFilter),
    [activeFilter]
  )

  const filteredEdges = useMemo(() => {
    const nodeIds = new Set(filteredNodes.map((n) => n.id))
    return PLACEHOLDER_EDGES.filter(
