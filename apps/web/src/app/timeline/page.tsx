...
import type { TimelineEvent, TimelineEra } from "@/components/timeline/timeline-track"
...
const PLACEHOLDER_ERAS: TimelineEra[] = [
...
type FilterType = "all" | "concept" | "thinker" | "book" | "event"
...
export default function TimelinePage() {
  ...
  const filteredEvents = useMemo(
    () =>
      activeFilter === "all"
        ? PLACEHOLDER_EVENTS
        : PLACEHOLDER_EVENTS.filter((e) => e.type === activeFilter),
    [activeFilter]
  )

  return (
    <div className="mx-auto flex min-h-screen max-w-container-page flex-col px-6 pb-20 pt-8">
      <div className="mb-2">
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4 border-b border-border pb-4">
        ...
        <div className="flex items-center gap-1">
          ...
          {ZOOM_LEVELS.map((z) => (
            <button
              key={z.label}
              onClick={() => setActiveZoom(z.years)}
              className={cn(
                "rounded-md px-2 py-0.5 text-meta font-medium transition-colors duration-[var(--motion-fast)]",
                activeZoom === z.years
                  ? "bg-primary/10 text-primary"
                  : "text-text-muted hover:bg-elevated hover:text-text"
              )}
