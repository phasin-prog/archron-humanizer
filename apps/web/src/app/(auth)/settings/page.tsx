...
export default function SettingsPage() {
  ...
  const [expertise, setExpertise] = useState<string[]>(["Psychology", "Philosophy", "Religion"])
  const [isPublic, setIsPublic] = useState(true)
  const [emailPrefs, setEmailPrefs] = useState<string[]>(["review-requests", "mentions"])
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3">
                  {DOMAINS.map((domain) => (
                    <Checkbox
                      ...
                      id={`expertise-${domain}`}
                      label={domain}
                      checked={expertise.includes(domain)}
                      onChange={() => toggleExpertise(domain)}
                    />
                  ))}
                </div>
              </div>
            ...
            <CardContent className="space-y-3">
              {EMAIL_PREFS.map((pref) => (
                <Checkbox
                  ...
                  id={`email-${pref.id}`}
                  label={pref.label}
                  checked={emailPrefs.includes(pref.id)}
