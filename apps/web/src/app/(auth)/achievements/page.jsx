...
import { Card, CardContent, Badge, Tabs, TabsList, TabsTrigger, TabsContent, Progress, } from "@archron/ui";
...
const CATEGORY_ORDER = ["milestone", "quality", "domain", "community"];
...
function AchievementBadge({ achievement }) {
    ...
    return (<Card className={`border-border transition-colors ${earned
            ? "hover:border-primary/30 bg-card"
            : "opacity-40 border-dashed"}`}>
      <CardContent className="flex flex-col items-center gap-2 pt-6 text-center">
        <div className={`flex h-14 w-14 items-center justify-center rounded-full text-xl font-bold ${earned
      ...
      <div className="mx-auto max-w-container-page px-6 pb-24 pt-24">
        ...
        <section className="mt-8">
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v)}>
            ...
            return (<TabsTrigger key={cat} value={cat}>
                    {CATEGORY_LABEL[cat]}
            ...
            {CATEGORY_ORDER.map((cat) => (<TabsContent key={cat} value={cat}>
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-4">
                  {filtered.map((achievement) => (<AchievementBadge key={achievement.id} achievement={achievement}/>))}
                </div>
              </TabsContent>))}
          </Tabs>
        </section>
