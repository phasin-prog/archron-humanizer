...
import { Card, CardContent, Badge, Progress, Divider, EditorIcon, UploadIcon, KnowledgeArticleIcon, KnowledgeConceptIcon, } from "@archron/ui";
const MOCK_STATS = {
    reputation: 485,
    publishedCount: 12,
    currentDrafts: 3,
        ...
        <div className="mt-12 grid grid-cols-1 gap-8 lg:grid-cols-3">
          <section className="lg:col-span-2">
            ...
            <div className="space-y-3">
              {MOCK_DRAFTS.length === 0 && (<Card>
                  <CardContent className="py-8 text-center">
                    <p className="text-body text-text-muted">No drafts yet</p>
                  </CardContent>
                </Card>)}
              {MOCK_DRAFTS.map((draft) => (<Card key={draft.id} className="transition-colors duration-[var(--motion-normal)] hover:border-primary/30">
          ...
          <section>
            ...
            <Card>
              <CardContent className="py-4">
                {MOCK_ACTIVITY.length === 0 && (<p className="py-4 text-center text-caption text-text-muted">No recent activity</p>)}
                <div className="space-y-3">
                  {MOCK_ACTIVITY.map((item, i) => (<div key={item.id}>
