...
export interface TimelineEntryProps {
  ...
  title: string
  description?: string
  significance?: "major" | "minor" | "contextual"
  ...
  return (
    <div className={cn("relative flex gap-4 pb-8", className)}>
      <div className="flex flex-col items-center shrink-0">
        <div
          className={cn(
            "size-3 rounded-full border-2 border-primary bg-background",
            significance === "major" && "bg-primary",
          )}
        />
          ...
          {relatedThinkers.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-2">
              {relatedThinkers.map((thinker) => (
                <Badge key={thinker} variant="outline" className="text-xs">
                  {thinker}
                </Badge>
              ))}
            </div>
