...
import { BookOpen, FileText, Lightbulb, User, School, } from "lucide-react";
const typeConfig = {
...
export function KnowledgeCard({ slug, title, objectType, description, backlinkCount, viewCount, className, onClick, }) {
    ...
    return (<Card className={cn("group cursor-pointer transition-shadow hover:shadow-md", className)} onClick={onClick ?? (() => { window.location.href = `/${slug}`; })}>
      ...
      {(backlinkCount !== undefined || viewCount !== undefined) && (<CardFooter className="flex items-center gap-4 text-xs text-muted-foreground">
          {backlinkCount !== undefined && (<span>{backlinkCount} backlinks</span>)}
