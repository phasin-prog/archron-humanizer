...
function Command({ children, className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  ...
  return (
    <CommandContext.Provider value={{ search, setSearch, open, setOpen }}>
...
function CommandDialog({ children, className, ...props }: CommandDialogProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center pt-[20vh]">
      ...
      <div
        className={cn(
          "relative z-50 w-full max-w-lg rounded-lg border bg-surface shadow-lg",
          className,
        )}
        {...props}
      >
...
function CommandInput({ className, placeholder = "Type a command or search...", ...props }: React.InputHTMLAttributes<HTMLInputElement>) {
  ...
  return (
    <div className="flex items-center border-b px-3">
      ...
      <input
        className={cn(
          "flex h-11 w-full rounded-md bg-transparent py-3 text-body outline-none placeholder:text-muted-foreground",
          className,
        )}
        value={search}
...
function CommandEmpty({ children, className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={cn("py-6 text-center text-caption text-muted-foreground", className)} {...props}>
      {children}
    </div>
  )
}
...
function CommandItem({ children, className, onSelect, ...props }: CommandItemProps) {
  const { setOpen } = useCommand()
  return (
    <div
      className={cn(
        "relative flex cursor-pointer select-none items-center gap-2 rounded-sm px-2 py-1.5 text-body outline-none hover:bg-muted transition-colors",
        className,
      )}
      onClick={() => {
        onSelect?.()
