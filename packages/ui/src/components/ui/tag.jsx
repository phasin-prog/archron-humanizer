...
const tagVariants = cva("inline-flex items-center rounded-md px-2 py-0.5 text-xs font-medium", {
    variants: {
        objectType: {
...
export function Tag({ className, objectType, label, ...props }) {
    return (<span className={cn(tagVariants({ objectType }), className)} {...props}>
      {label}
    </span>);
}
export { tagVariants };
