...
export function WikiLinkAutocomplete({ editor, onSearch, }) {
    ...
    const insertWikiLink = useCallback((item) => {
    ...
    return (<>
      ...
      <div className="fixed z-50 w-80 rounded-lg border bg-surface shadow-lg overflow-hidden" style={{ left: position.x, top: position.y }}>
        <div className="max-h-64 overflow-y-auto p-1">
          {results.length === 0 && (<div className="px-2 py-3 text-caption text-muted-foreground text-center">
              {query ? "No matches" : "Start typing..."}
            </div>)}
          {results.map((item, i) => (<button key={item.slug} onClick={() => insertWikiLink(item)} className={cn("flex w-full items-center gap-2 rounded-sm px-2 py-1.5 text-left hover:bg-muted transition-colors", i === selectedIndex && "bg-muted")}>
